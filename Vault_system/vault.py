...
class Vault:
    ...
    def sample_mpws(self, num):
        sampled_mpws = []
        for _ in range(num):
            bid_ = randint(0, self.T - 1)
            mpw_sampled = choice(self.decopmpws_init[bid_])
            sampled_mpws.append(mpw_sampled)
        return sampled_mpws
    
    def init_decoympws(self):
        decoympws = [line.strip() for line in
         open(SOURCE_PATH+'/Vault_system/10_xato_prepared.txt', 'r') if
         len(line.strip()) > 6]
        self.decopmpws_init = defaultdict(list) # mpwe prepared for initiliazation
        self.mpw_pool = decoympws # len = 5189402
    ...
    def encrypt_block(self, pw_block_old, pw_block_new, use_multithread=False):
        ...
        if use_multithread:
            ...
            ts__ = time()
            for w in works:
                mc_, independent_ind, ts___, correct_mpw_batch = que.get()
                ...
                # 将子进程返回的correct_mpw合并到主进程的列表中
                self.correct_mpw.extend(correct_mpw_batch)
            if len(pw_block_new) - len(pw_block_old) == 1:
    ...
    def dec_any_mpw(self, mpw_try):
        ts = time()
        Mc_id = get_hash(mpw_try) % self.T
        if Mc_id != self.real_vault_id:
            print("Wrong mpw!")
        else:
            print("Correct mpw!")
        ct_block = self.MC[Mc_id]
        pw_block = self.decrypt_block(ct_block)
        print("Decrypted pw block:", pw_block)
        if self.real_pw != pw_block:
            print('unequal_load results!')
        return time() - ts
...
def encrypt_ithblock(que, ts__, new_pws, block_id_start, block_id_end, pw_block_new, decoympws, independent_prob_list, allpws_spmprob, real_vault_id, mpw, T, salts):
    ...
    for block_id in tqdm(range(block_id_start, block_id_end)):
        for ith_newpw, pw in enumerate(new_pws):
            ...
            local_correct_mpw.append(mpw_tmp)
    ...
    que.put((incre_mc, [fg for fg in independent_indicator if fg != -1], time(), local_correct_mpw))
...
def test_system_cost(vs_list, T_list, rn):
    ...
    :return:
    """
    with open(SOURCE_PATH + '/Vault_system/samplevaults.json','r') as f:
...
def guess_test():
    ...
    for pwv in pwvaults:
        if len(pwv) == 20:
            selected_vaults.append(pwv)
    vault_t = Vault('qnmeu12n90s1', '124871', T=1000)
    vault_t.init_vault(createvault_frompws(selected_vaults[0]))
    mpw_try = 'qnmeu12n90s1'
    single_guess_dec(vault_t, mpw_try)

def single_guess_dec(vault: Vault, mpw_try):
    tused = vault.dec_any_mpw(mpw_try)
    print("Time used for guessing mpw {} is {}s".format(mpw_try, tused))
...
#     T_list = [4000] #[1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000] # T_list=
#     test_system_cost(vs_list=vs_list, T_list=T_list, rn=1) # rn=20
