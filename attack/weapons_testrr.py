...
import random
import subprocess
from weight_testrr import obtain_metricdifference, obtain_vaultmetrics_multiprocess, obtain_vaultmetrics
import math
        ...
        for threa in range(N_EXP_VAULTS if not args.withleak else T_PHY):
            ...
            if threa % REFRESH_RATE == 0:
                ...
                if args.victim == 'MSPM':
                    ...
                    decoyvaults, probs_spm_mspm = generate_batchdecoys(tset, DV.leakpw, decoynum=REFRESH_RATE)#generate_batchdecoys(tset, None, decoynum=REFRESH_RATE)#
                ...
                decoyidx2metrics = obtain_vaultmetrics_multiprocess(decoyvaults)
                decoyvaults.extend(decoyvaults_)

                if threa == 0 and args.victim != 'PCFG':
                    vault = tset
                    ...
                    metrics_decoy = None
                elif threa == 0 and args.victim == 'PCFG':
            ...
            if threa > 0:
                ...
                metrics_decoy = decoyidx2metrics[threa % REFRESH_RATE]
            ...
            results['metrics_rank'] = obtain_metricdifference(weight.learned_vaultmetric[len(vault)], vault, metrics_decoy) if kl != -np.inf else -np.inf
            results['vault'] = vault
...
def addition_weight(batch_bundle):
    ...
    with open(SOURCE_PATH + '/attack/credtweak/credTweakAttack/data/pass2path_1667500_dataset_ts'+sfx+'.pkl', 'rb') as f:
