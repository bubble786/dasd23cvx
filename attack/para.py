...
def check_pin_batch(reshuffled_list, vaults, DVs, real=False, gpuid=0): # 'vaults' is a list (batch) of vault, 'DVs' is a list (batch) of DV
    ...
    # zzp dubug
    # print("--- Debugging check_pin_batch ---")
    # for i, (dv_obj, vault) in enumerate(zip(DVs, vaults)):
    #     print(f"Item {i}:")
    #     print(f"  DV object's leakpw: {dv_obj.leakpw}")
    #     print(f"  Is leakpw in current vault? {dv_obj.leakpw in vault}")
    ...
    if not all([realleakpwid in dvspwids for dvspwids in dv_pwids]):
        print("error occur:all([realleakpwid in dvspwids for dvspwids in dv_pwids])")
        print(f"realleakpwid: {realleakpwid}\ndvs_pwids: {dv_pwids}\n")
