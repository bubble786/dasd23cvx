...
class Measure:
    ...
    def extract_data(self, pnlst, tlst, num_processes, plot_intersec, expdata_path, repeatn=10,
                     destination = SOURCE_PATH + '/results/MSPM/extracted_results/tmp/'):
    ...
    def rank_r(self, results_threa, softs_threa=None, pn4=True, reshuedidx=None, gpuid=0, seed_shot=0):
        ...
        print(f"len results_threa: {len(results_threa)}")
        for i, result_batch in enumerate(results_threa): # result_batch: [results_avault_aiter, pin_frequency_lists]
            ...
            vaults.append(vault)
            #assert [results_threa[0][0][ith][w] for ith in range(len(results_threa[0][0])) for w in ['psp', 'pps', 'phybrid', 'kl', 'wang_single', 'wang_similar', 'wang_hybrid', 'MS']] == [pr for pr_a in prior_array for pr in pr_a]
    ...
    def find_real_candidatelist(self, prior_array, idx, logis, intraoffset, batch_rank, vaultsize, reshuedidx=None, gpuid=0):
        ...
        # 测指定rank,指定某一个rank,比如hybrid->2,如果不指定,注释该行
        # median_rankid = 7 # 7 for test metric
...
def main(plot_intersec = False):
    ...
    expdata_path = SOURCE_PATH + \
                    "/attack/results/MSPM/bubble_online/" +\
    ...
    if len(args.outdatapath) > 0:
        ...
        measure.extract_data(pnlst, tlst, num_processes, plot_intersec, expdata_path, repeatn=repeatn, destination = SOURCE_PATH + '/attack/results/MSPM/extracted_results/fixeditv_Nitv13/') # vanilla, vanilla_intersec, fixeditv_Nitv13, fixeditv_Nitv13_intersec

if __name__ == '__main__':
    #main()
    import json
    # merge two json files into one
    jsonpath1 = SOURCE_PATH + '/attack/results/MSPM/synthesis_vaults/vaultsize60/vaults (copy).json'
    jsonpath2 = SOURCE_PATH + '/attack/results/MSPM/synthesis_vaults/vaultsize60/vaults.json'
