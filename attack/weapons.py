...
from MSPM.incre_pw_coding import Incremental_Encoder, gen_n_password_random
...
def mspmattack(rn, outputdir, T, batch, x, testset, decoygen, dte, weight, measure, vaultgen, assem_pws, vids, incre_encoder, dtimes, btg = None, mpwset=None):
    """
    ...
    :param mpwset: dict about mpwset,foramt-rockyou[pw] = count
    ...
    for n_, tset in enumerate(tqdm(testset)): # tset is a password vault (list of passwords) from dataset
        ...
        for threa in range(N_EXP_VAULTS if not args.withleak else T_PHY):
            ...
            if threa % REFRESH_RATE == 0:
                ...
                if args.victim == 'MSPM':
                    ...
                    batch_num = 1
                    probs_spm_mspm, decoyvaults = [], []
                    for ith_decoybath in range(batch_num):

                        _, probs_spm_mspm_, decoyvaults_ = vaultgen(len(tset) - 1, REFRESH_RATE, dte=dte, path1pws=None, path1probs=None, pastepws=tset, pasteprobs=pasteprobs, threshold=threshold, possibleidx=DV.possibleidx) #vaultgen(len(tset) - 1, int(REFRESH_RATE/batch_num), dte=dte, path1pws=None, path1probs=None, pastepws=tset, pasteprobs=pasteprobs, threshold=[0 for thr in threshold], possibleidx=[1]) #
                    ...
                    for ith_decoybath in range(batch_num):
                        ...
                        decoyvaults_, probs_spm_mspm_ = generate_batchdecoys(tset, DV.leakpw, decoynum=REFRESH_RATE)#generate_batchdecoys(tset, None, decoynum=int(REFRESH_RATE/batch_num))#
                    ...
                    while True:
                        try:
                            pcfgdte = DTE(pcfg.decode_grammar(list(np.random.randint(0, MAX_INT, 500))))
                            break
                        except:
                            continue
                    vaultseed = pcfgdte.acquire_realvault_seed(tset)
                    decoyvaults, probs_spm_mspm = pcfgdte.decode_decoyvaults(vaultseed, 500, 100, len(tset))

                    randomseeds = cp.random.randint(low=0, high=SEED_MAX_RANGE, size=(500 + 100 * len(tset)) * (REFRESH_RATE+PS_DECOYSIZE)*2).get() # *1.3 because some error for generating some decoy vaults
                    decoyvaults_, probs_spm_mspm_ = pcfgdte.decode_decoyvaults(randomseeds, 500, 100, len(tset))
                
                elif args.victim == 'sgf':
                    print(f"once attacker guessing mpw based on sgf start")
                    # 1-init dte
                    sgf_T = T
                    real_pws = tset
                    encoder = incre_encoder
                    # gen by MSPM encoder with random seeds, size 2n
                    # 16 sg pws{realvault,dummy vault}
                    len_real = len(real_pws)
                    dummy_ratio = dtimes #8, 16, 32, 64
                    dpw_cnt = len_real * dummy_ratio
                    print(f"dummy/real ratio : {dummy_ratio}")
                    dummy_pws = gen_n_password_random(real_pws, encoder, dpw_cnt)
                    dum_pws = dummy_pws
                    print(f"real_pws: {real_pws}\nlen real pws: {len(real_pws)} unique real_pw cnt: {len(set(real_pws))}")
                    print(f"dummy_pws: {dummy_pws}\nlen dummy pws: {len(dummy_pws)} unique dummy_pw cnt: {len(set(dummy_pws))}")
                    intersection_attack_result_dir = outputdir + 'intersection_attack_result/'

                    sgf_dte = sgf(T=sgf_T, real_pws = real_pws, dummy_pws=dummy_pws, intersection_attack_result_dir=intersection_attack_result_dir, mpwset=mpwset)
                    
                    # 步骤1: 构造vault system并收集MPW
                    sgf_dte.setup_vault_system(sgf_T, btg=btg)
                ...
                # 保存probs_spm_mspm和decoyvaults到本地文件,仅调试用
                decoy_dir = os.path.join(SOURCE_PATH, 'attack', 'decoy_cache', args.victim, args.tag)
                os.makedirs(decoy_dir, exist_ok=True)
                decoy_data_path = os.path.join(decoy_dir, f'{args.victim}_decoy_data_v{n_}_vid{vids[n_]}.json')

                save_decoy_data(tset, dum_pws, fixed_leadpw, probs_spm_mspm, decoyvaults, decoy_data_path)
                print(f"decoy cache data saved to {decoy_data_path}, len probs: {len(probs_spm_mspm)}, len decoyvaults: {len(decoyvaults)}")
                ...
                e_threa_0_rank = time()
                # print(f'threa 0 rank using time: {e_threa_0_rank - s_threa_0_rank}s')
            if threa > 0:
                # PS_DECOYSIZE decoy vaults used for similarity weight calculation
                #s_ = time()
                s_threa_n0_rank = time()
                ...
                if results['psp'] == 0:
                    ...
                    kl = weight.kl(vault, dte.spm, prob_tmp)
                    wang_singlepass = weight.wang_singlepass(vault, dte, prob_tmp)
                e_threa_n0_rank = time()
                # print(f'threa n>0 rank using time: {e_threa_n0_rank - s_threa_n0_rank}s')
            ...
            e_rank_c = time()
            # print(f'Once calc rank c using time: {e_rank_c - s_rank_c}s')
        ...
        e_rank_r = time()
        # print(f'rank_r using time: {e_rank_r - s_rank_r}s')
...
def save_decoy_data(tset, dummy_pws, fixed_leadpw, probs, vaults, output_path):
    """
    使用 json 将概率和诱饵保险库保存到可视化的本地文件中。

    :param probs: 概率列表 (probs_spm_mspm)
    :param vaults: 诱饵保险库列表 (decoyvaults)
    ...
    try:
        ...
        for i in range(len(vaults)):
            prob_vault = []
            prob_in_vault = probs[i * len(tset):(i + 1) * len(tset)]
            cur_vault = vaults[i]
            prob_vault = list(zip(cur_vault, prob_in_vault))
            prob_vaults.append(prob_vault)
        
        if args.victim == 'sgf':
            data_to_save = {
                'tset': tset,
                'dummy_pws': dummy_pws,
                'fixed_leak_password': fixed_leadpw,
                'len prob': len(probs),
                'len vaults': len(vaults),
                'prob_vaults': prob_vaults
            }
        else :
            data_to_save = {
                'tset': tset,
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(data_to_save, f, indent=4, cls=NpEncoder, ensure_ascii=False)
        print(f"诱饵数据已成功保存到: {output_path}")
    except Exception as e:
        print(f"保存诱饵数据时出错: {e}")

def list_shuffle(mpw, pin, vault_size, recover=False, existing_list=None):
    """
    打乱vault索引,以pin位随机数种子
