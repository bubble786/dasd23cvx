...
def plot_multiple_rank_cdf(csv_files_dict, output_path=None, title="BC50"):
    ...
    # ============== 字体大小统一设置区域（可在此处统一调整所有字体大小）==============
    GLOBAL_FONTSIZE = 20  # 全局基础字体大小（可修改此值来统一调整字体）
    ...
    for label, csv_path in csv_files_dict.items():
        try:
            # 读取数据
...
def draw_cdf(dataset_name, base_dir):
    ...
    # 设置输出路径
    output_path = os.path.join(base_dir, f'{dataset_name}_cdf_comparison_1.png')
