...
def main():
    ...
    root = SOURCE_PATH #'/Users/sunbin/Library/CloudStorage/'
    write_dir = root + '/attack/results_handling/table_data/'
    ...
    for settings in expermented_settings:
        ...
        results = []
        # read_dir = root + '/attack/results/MSPM/' + tstset + '/vaults_cmr_pl8/' #
        read_dir = '/home/zzp/lab/bubble/bubble-v2/Bubble-fixed2/attack/results/sgf/bc50/onefold_testset/attack_result_10_testdataid0_cons1_pin4'
        read_dir = [d[0] for d in os.walk(read_dir)]
...
def extract_login_distribution(results, nol_ith, write_dir, intersec=False, n_vaulteachshot=75, delta=10, chengorgolla=0):
    ...
    # re = pd.read_csv(SOURCE_PATH + '/attack/results_handling/valuewise_plcmr_bc100_pl8.csv', sep=',', header=None)
    re = pd.read_csv(SOURCE_PATH + 'attack/results_handling/table_data/valuewise_plcmr_bc50_pl8.csv', sep=',', header=None)
    ...
    np.savetxt(SOURCE_PATH + "/attack/results_handling/table_data/errorplot_pl.csv", np.stack([errorplot_mean, errorplot_std], axis=1), delimiter=",")
    ...
    # write to csv error data
    np.savetxt(SOURCE_PATH + "/attack/results_handling/table_data/errorplot_cmr.csv",
