...
def select_testset_bycr():
    with open('/home/dataset/BreachCompilation/fold_0_1.json', 'r') as f:
        ...
        for k, v in itms:
            if np.abs(len(v)-selectsize) > select_range or round(len(v) / len(set(v))) not in toselect_rr: # round(len(v) / len(set(v)))
                ...
                if len(toselect_rr) == 0:
                    break
    with open(SOURCE_PATH + '/attack/results/MSPM/bc/vaults_size55_differentleaksforvault/fold_0_1.json', 'w') as f:
        json.dump(testset, f)
    ...
    for k, v in tqdm.tqdm(itms):
        ...
        if int(v_beta) in select_beta:
            ...
            if len(select_beta) == 0:
                break
    print(select_beta)
    with open(SOURCE_PATH + '/attack/results/MSPM/bc/fp_exp/fold_0_1.json', 'w') as f:
        json.dump(testset, f)


def select_testset_byvs():
    with open('/home/dataset/BreachCompilation/mixed_groups_2_200_minits.json', 'r') as f:
        vaultdict = json.load(f)
    testset = {} # rr 2 ~ 85
    itms = list(vaultdict.items())
    random.shuffle(itms)

    with open('/home/dataset/BreachCompilation/pastebin_ts.json', 'r') as f:
    ...
    sample_set = {}
    for k, v in itms:
        if len(v) > 200 or vs2num[len(v)] >= 30: # len(v) not in vaultsize2select: #
            continue
        vs2num[len(v)] += 1
        sample_set[k] = v
    ...
    for k, v in samples:
        ...
        if len(testset) == 100:
            break
    testset = testset
    '''with open(SOURCE_PATH + '/attack/results/MSPM/bc200/mversion/fold_0_1.json', 'w') as f:
        json.dump(testset, f)'''


def select_testset_byrrcm():
    with open('/home/dataset/BreachCompilation/mixed_groups_2_50_ts.json', 'r') as f:
        vaultdict = json.load(f)
    testset = {}  # rr 2 ~ 85
    itms = list(vaultdict.items())
    random.shuffle(itms)
    ...
    with open(SOURCE_PATH + '/attack/results/MSPM/bc50/vaults_pl_cmr015_025/fold_0_1.json', 'w') as f:
        json.dump(testset, f)
    ...
    for k, v in itms:
        ...
        for totuple in toselect_tuple:
            if round(mr_rcm[0]) == totuple[0] and round(mr_rcm[1], 1) == totuple[1] and mr_rcm[1] != 0: # get group idx
                ...
                break
    with open(SOURCE_PATH + '/attack/results/MSPM/bc/vaults_4groups/fold_0_1.json', 'w') as f:
        json.dump(testset, f)
...
def synvaults2testset():
    ...
    target_path = SOURCE_PATH + '/attack/results/MSPM/bc/vaults_rr15_vswise/fold_0_1.json'
...
def duplcate_testset(ntimes):
    ...
    with open(SOURCE_PATH + '/attack/results/MSPM/bc/vaults_size55_differentleaksforvault/fold_0_1_copy.json', 'r') as f:
        testset_ = json.load(f)
    testsetpath = SOURCE_PATH + '/attack/results/MSPM/bc/vaults_size55_differentleaksforvault/fold_0_1.json'
...
def testset_join():
    testset_path1 = SOURCE_PATH + '/attack/results/MSPM/bc/vaults_plcmr/fold_0_1.json'
    with open(testset_path1, 'r') as f:
        testset1 = json.load(f)
    testset1_coy = copy.deepcopy(testset1)
    testset_path2 = SOURCE_PATH + '/attack/results/MSPM/bc/vaults_plcmr/newselect.json'
...
def testset_prune():
    testset_path = SOURCE_PATH + '/attack/results/MSPM/bc/fp_exp/fold_0_1.json'
    ...
    # process each folder
    for folder in os.listdir(SOURCE_PATH + '/attack/results/MSPM/bc/vaults_plcmr/'):
        if 'attack' not in folder:
            continue
        folderpath = os.path.join(SOURCE_PATH + '/attack/results/MSPM/bc/vaults_plcmr', folder)
        removed_files = 0
...
if __name__ == "__main__":
    with open(SOURCE_PATH + '/attack/results/MSPM/bc50/onefold_testset/fold_0_1.json', 'r') as f:
        pbvaults = json.load(f)
    ...
    with open(SOURCE_PATH + '/attack/results/MSPM/bc200/moreversions/fold_0_1.json', 'w') as f:
    ...
    dir_path = SOURCE_PATH + '/attack/results/MSPM/pb/testset/attack_result_1000_testdataid0_cons1_pin3'
    re = []
    for shotid in range(1):  # [2]:#
        for vid in range(138): #(list(range(25))+list(range(69,138))):#
            filename = 'results_v' + str(vid) + '_shot' + str(shotid) + '.data'
            with open(os.path.join(dir_path, filename), 'rb') as f:
                r = pickle.load(f)
                re.append(r)
    print(np.array([r[1][0][0][0]['r_three'] for r in re]).mean(axis=0))
    print((np.array([r[1][0][0][0]['r_three'] for r in re]) == 0).sum(0) / len(re))
    ...
    n_rows, n_cols = 1,1
    blocked = np.genfromtxt(SOURCE_PATH + '/attack/results_handling/table_data/blocked.csv', delimiter=',')
    failed = np.genfromtxt(SOURCE_PATH + '/attack/results_handling/table_data/fail.csv', delimiter=',')
    fp = failed - blocked
    print('avg blocked:', blocked.mean(axis=0))
    print_nxm(blocked.mean(axis=0), n_rows, n_cols)
    np.savetxt(SOURCE_PATH+"/attack/results_handling/table_data/blocked_avg.csv", blocked.mean(axis=0).reshape(n_rows, n_cols), delimiter=",")
    print('avg fp:', fp.mean(axis=0))
    print_nxm(fp.mean(axis=0), n_rows, n_cols)
    np.savetxt(SOURCE_PATH+"/attack/results_handling/table_data/fp_avg.csv", fp.mean(axis=0).reshape(n_rows, n_cols), delimiter=",")
    print('avg failed:', failed.mean(axis=0))
    print_nxm(failed.mean(axis=0), n_rows, n_cols)
    np.savetxt(SOURCE_PATH+"/attack/results_handling/table_data/failed_avg.csv", failed.mean(axis=0).reshape(n_rows, n_cols), delimiter=",")
    ...
    re = pd.read_csv(SOURCE_PATH+'/attack/results_handling/valuewise_plcmr_bc50_cmr02.csv', sep=',', header=None)
    re = re.values
