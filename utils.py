...
class ExtendVault:
    def __init__(self):
        with open('/home/crack_tools/wordlist/10_xato.txt', 'r') as f:
...
def get_Mefp_candidatelist_para(logis, r, intraoffset, vaultsize, reshuedidx, gpuid):
    ...
    print(f"len bsz_list:{len(bsz_list)}")
...
class Digit_Vault:
    def __init__(self, real_vault, vid=0, originsize=None):
        ...
        random_fixed = 0 # add by zzp , test randomnees influence on attack result
        
        if random_fixed == 1:
        # vid as random seed,fixed randomness
            ...
            self.leakpw = random.choice(real_vault_tmp[:-1] if args.intersection else real_vault_tmp) # fixed leakpw for each vault (randomness control)
        ...
        # vid as random seed,fixed randomness
            ...
            random.shuffle(self.possibleidx)
...
def double_testvaults(results):
    ...
    with open(SOURCE_PATH + '/attack/results/MSPM/pastebintest_expanded/expaneded_vaults.json', 'w') as f:
        json.dump(decoy_vaults, f)
...
def multidouble_selectedvault(results, unique_vids):
    ...
    with open(SOURCE_PATH + '/attack/results/MSPM/pastebintest_expanded/expaneded_avault.json', 'w') as f:
        json.dump(decoy_vaults, f)
...
def multisamesize_selectedvault(results, unique_vids):
    ...
    for idx in unique_vids:
        ...
        if len(vault) == 10:
            ...
            with open(SOURCE_PATH + '/attack/results/MSPM/synthesis_vaults/vaultsizeany/vaults.json', 'w') as f:
                json.dump(decoy_vaults, f)
...
def get_rrandmr(jsonpath, rr_cmr, rr_cmr_sep, write_lhm=False):
    ...
    # write to csv file
    with open(SOURCE_PATH + '/attack/results_handling/table_data/rrandmr.csv', 'w') as f:
        writer = csv.writer(f)
    ...
    if write_lhm:
        results = rr_cmr
        fail = pd.read_csv(SOURCE_PATH + '/attack/results_handling/table_data/fail.csv', sep=',', header=None).values # table_data/rrandmr.csv len(unique_vs) x (num of experiments with different paramenters)
        blocked = pd.read_csv(SOURCE_PATH + '/attack/results_handling/table_data/blocked.csv', sep=',', header=None).values
...
def main():
    ...
    re = pd.read_csv(SOURCE_PATH + '/attack/results_handling/valuewise_plcmr_newtestset.csv', sep=',', header=None) #table_data/rrandmr.csv
    re = re.values
