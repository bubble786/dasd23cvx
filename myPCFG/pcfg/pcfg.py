#!/usr/bin/python
# run this script directly: python -m pcfg.pcfg
"""
...
class TrainedGrammar(object):
    ...
    def __init__(self, g_file=hny_config.TRAINED_GRAMMAR_FILE, cal_cdf=False):
        ...
        self.pad_y1_PCFG() # 改进:填充Y1缺失规则['"', '|', '}', '\\']
        
    def pad_y1_PCFG(self):
        """
    ...
    def max_parse_tree(self, word):  # leftmost parse-tree
        pt = ParseTree()
        p = self.parse(word)
    ...
    def l_parse_tree(self, word):  # leftmost parse-tree
        pt = ParseTree()
        p = self.parse(word)
        # print("max parse for {}: {}".format(word, p))
        if not p:
            print("Failing at {!r}".format(word))
            return pt
    ...
    def encode_pw(self, pw):
        ...
        code_g.extend(convert2group(0, 1, extra)) # 填充[0, MAXINT]随机数到PASSWORD_LENGTH
        return code_g
    ...
    def decode_rule_withprob(self, l, p):
        rhs_dict = self.G[l]
        if not rhs_dict:
            return ''
        assert '__total__' in rhs_dict, "__total__ not in {!r}, l={!r}" \
    ...
    def decode_l33t_withprob(self, w, iterp):
        # print("L33t:::", w, iterp)
        l, prob = self.decode_rule_withprob('L', next(iterp))
        if l == 'Caps':
            ...
            nw = ''
            for c in w:
                nwi, probi = self.decode_rule_withprob('L_%s' % c, next(iterp))
                nw += nwi
                prob += probi
    ...
    def decode_pw(self, P):
        ...
        while stack:
            lhs = stack.pop()
            p_value = next(iterp)
            # print(f"Step {step}: lhs='{lhs}', p={p_value}, stack_size={len(stack)}")
            rhs = self.decode_rule(lhs, p_value)
            step += 1
            # lhs = stack.pop()
    ...
    def decode_pw_withprob(self, P):
        ...
        while stack:
            lhs = stack.pop()
            rhs, pathprob = self.decode_rule_withprob(lhs, next(iterp))
            ...
            if lhs in ['G', 'T', 'W', 'Y', 'D']:
                ...
                rhs, pathprob = self.decode_l33t_withprob(rhs, iterp)
                plaintext += rhs
                prob_plaintext += pathprob
            else:
                plaintext += rhs
...
class SubGrammar(TrainedGrammar):
    def __init__(self, base_pcfg):
        self.cal_cdf = False
        self.valid_rule = defaultdict(int)  # 默认值为0
        R = RuleSet()
    ...
    def finalize(self):
        ...
        if 'T' in self.G:
            self.date = Date(T_rules=[x
                                      ...
                                      if x != '__total__'])
        self.freeze = True # 冻结语法，禁止添加新规则
        self.R.G.default_factory = None # 禁用 OrderedDict 的默认工厂函数,防止意外添加新rule
    ...
    def update_grammar(self, *args):
        self.reset() # 删除每个rule set的频率总和,finalize()重新计算
        for pw in args:
            pw = pw.replace('\\', '')
            # new直接采用最优解析,新rule赋值随机频数,保证全新pw也能记录grammar并高概率出现
            rs = RuleSet()
            pt = self.base_pcfg.max_parse_tree(pw)
            for p in pt.tree:
                # 频数从[1000,10000]随机采样
                # f = random.randint(1000, 10000) # 随机填充最低频数,参数可调,第一版
                f = random.randint(1000000, 1100000)
                rs.add_rule(*p, f=f) # *表示解包,将元组p拆分为两个参数
                # 添加当前(l,r)到 valid_rule
                self.valid_rule[p] = 1  # 标记为已添加
                 
            # old基于base_pcfg的规则，采用局部最优解析，可能为逐字符解析
            # rs = self.base_pcfg.rule_set(pw)
            
            # print("max parse for {}: {}".format(pw, pt))
            self.R.update_set(rs, with_freq=False)
        self.finalize()

    # 构造SubGrammar时如果需要调整rule的频率，需要在这里相应修改
    def fix_freq(self):
        for l, v in list(self.R.items()):
            s = 0
            # 在base频数和当前随机采样频数之间取最大值
            for r in v: # 遍历每个非终结符的右部,以orderdict的键形式
                if r != '__total__':
                    # 如果频率已经被手动设置为较高值，保留它
                    current_freq = v.get(r, 0)
                    base_freq = self.base_pcfg.get_freq(l, r)
                    # 保留较高的频率值
                    v[r] = max(current_freq, base_freq)
                    # v[r] = base_freq
            # 将当前rule set内valid_rule的频数设为较大值
            max_freq = max(v.values(), default=0)
            for r in v:
                if (l, r) in self.valid_rule:
                    v[r] = max_freq + random.randint(0, 1000)  # 在最大频数基础上增加随机数
                    # print("Setting frequency for ({}, {}) to {}".format(l, r, v[r]))
            # 计算总频率
            for r in v: # 遍历每个非终结符的右部,以orderdict的键形式
                if r != '__total__':
                    s += v[r]
