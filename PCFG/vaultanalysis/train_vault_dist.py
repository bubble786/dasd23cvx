...
from helper import open_, getIndex, convert2group
from Crypto.Random import random
import honeyvault_config as hny_config
from honeyvault_config import NONTERMINAL, TERMINAL, MIN_COUNT
from honeyvault_config import MEMLIMMIT, STATIC_DIR
from collections import OrderedDict, defaultdict
from pprint import pprint
from pcfg.pcfg import TrainedGrammar, SubGrammar
from pcfg.lexer import NonT_length2classmap as ntlm
...
def cal_size_subG(base_pcfg, vault_set_file):
    #tdata = [(k, filter(lambda x: x, v)) for k,v in json.load(open(vault_set_file)).items() if len(filter(lambda x: x, v))>1]
    tdata = [(k, v) for k,v in json.load(open(vault_set_file)).items()]
    rm = []
    for x in tdata:
        k, v = x
        for p in v:
    ...
    for k,v in data.items():
        ...
        g.update_grammar(*v)
        res = [(nt, len(g[nt])-1) if nt in g else (nt, 0) for nt in NT]
...
def cal_stat( fds=[], fnames=[] ):
    ...
    for fn in fds:
        ...
        for k,v in json.load(fn, object_pairs_hook=OrderedDict).items():
            ...
            k = ','.join(str(x) for x in [ v["length"]]+ [max(v.get(nt,0),0) for nt in NT])
            if k not in V:
    ...
    for k,v in G.items():                              
        if len(v)<30:                 
            for i in range(1,len(v)+30):   
                v[i] = 5*v.get(i, 1) 
    json.dump(G, open(STATIC_DIR+'vault_dist_pb.cfg', 'wb'),indent=2, separators=(',',':'), sort_keys=True)
