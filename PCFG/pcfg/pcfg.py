...
import json
from PCFG.pcfg.lexer_helper import Date, RuleSet, ParseTree
from PCFG.pcfg.lexer import NonT_L, get_nont_class
from PCFG.helper import (
...
class TrainedGrammar(object):
    ...
    def decode_rule(self, l, p, withprob=False):
        rhs_dict = self.G[l]
        if not rhs_dict:
            return ''
        ...
        for k, v in list(rhs_dict.items()):
            if p < v:
                return k, np.log(rhs_dict[k]/rhs_dict['__total__'])
            else:
                p -= v
        '''
        freqs = np.array(list(rhs_dict.values()), dtype=np.int64)
        cdf = np.cumsum(freqs)
        #idx = bisect.bisect_right(list(cdf), p)
        #dictitems = [[k, cdf[idx]] for idx, k in enumerate(list(rhs_dict.keys()))]
        idx = np.searchsorted(cdf, p, side='right') #np.array([0])# np.argmax((cdf - p) > 0)
        #threadsperblock = 32
        #blockspergrid = math.ceil(len(rhs_dict) / threadsperblock)
        #with cuda.gpus[0]:
        #    gpu_decode[blockspergrid, threadsperblock](p, 0, cdf, idx)
        idx = int(idx[0])'''

        #k = list(rhs_dict.keys())[idx]

        """freqs = cp.array(list(rhs_dict.values()), dtype=cp.int64)
        cdf = cp.cumsum(freqs)
        idx = cp.searchsorted(cdf, cp.asarray(p, dtype=cp.int64), side='right')
        idx = int(cp.asnumpy(idx))  # transfer back to CPU
        k = list(rhs_dict.keys())[idx]"""
    ...
    def decode_l33t(self, w, iterp):
        # print("L33t:::", w, iterp)
        l, prob = self.decode_rule('L', next(iterp))
        if l == 'Caps':
            return w.capitalize(), prob
        elif l == 'lower':
            return w.lower(), prob
        elif l == 'UPPER':
            return w.upper(), prob
            ...
            nw = ''
            for c in w:
                nwi, probi = self.decode_rule('L_%s' % c, next(iterp))
                nw += nwi
                prob += probi
            #nw = ''.join([self.decode_rule('L_%s' % c, next(iterp)) for c in w])
            return nw, prob

    def decode_pw(self, P, withprob=False):
        assert len(P) == hny_config.PASSWORD_LENGTH, \
            "Not correct length to decode, Expecting {}, got {}" \
                .format(hny_config.PASSWORD_LENGTH, len(P))

        iterp = iter(P)
        plaintext, prob_plaintext = '', 0
        stack = ['G']
        while stack:
            lhs = stack.pop()
            rhs, pathprob = self.decode_rule(lhs, next(iterp), withprob=withprob)
            prob_plaintext += pathprob
            if lhs in ['G', 'T', 'W', 'Y', 'D']:
                ...
                rhs, pathprob = self.decode_l33t(rhs, iterp)
                plaintext += rhs
                prob_plaintext += pathprob
            else:
                plaintext += rhs
        return plaintext, prob_plaintext
    ...
    def encode_grammar(self, G):
        ...
        while stack:
            ...
            code_g.extend([self.encode_rule(head, r) for r in rule_dict.keys() if r != '__total__'])
            # i = 0
    ...
    def decode_grammar(self, P):
        ...
        """
        g = SubGrammar(self) # copy special rules from the base grammar into subgrammar
        vd = VaultDistPCFG()
        iterp = iter(P)
        stack = ['G']
        done = list(g.default_keys())
        ts_decode_grammar = time.time()
        decode_rule_time = 0
        while stack:
            head = stack.pop()
            ...
            for _ in range(n):
                p = next(iterp)
                ts = time.time()
                rhs = self.decode_rule(head, p)[0]
                decode_rule_time += time.time() - ts
                if rhs != '__totoal__':
        ...
        # print("decode rule account for:", decode_rule_time / (time.time()-ts_decode_grammar))
        return g
...
class SubGrammar(TrainedGrammar):
    ...
    def finalize(self):
        ...
        if 'T' in self.G:
            self.date = Date(T_rules=[x for x in list(self.G['T'].keys()) if x != '__total__'])
        self.freeze = True
...
class VaultDistPCFG:
    ...
    def decode_vault_size(self, lhs, cf):
        ...
        if cf == 0:
            #print_err("Grammar of size 0!!!!\nI don't think the decryption will be right after this. I am sorry. Argument: (lhs: {}, cf: {})".format(lhs, cf))
            pass
...
if __name__ == '__main__':
    tg = TrainedGrammar()
    with open('/home/beeno/Dropbox/research_project/pycharm/nocrack-master/newcode/data/pastebin_ts.json', 'r') as f:
        vaults = json.load(f)
    maxid = max(vaults, key=lambda x: len(vaults[x]))
    g = SubGrammar(tg) # first copy special rule set from tg to subgrammar
    for pw in vaults[maxid]:
        tg.encode_pw(pw)
