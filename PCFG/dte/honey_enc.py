...
import string
import PCFG.honeyvault_config as hny_config
from PCFG.helper import getIndex, convert2group, random, whatchar
...
class DTE(object):
    ...
    def decode_pw(self, P):
        ...
        """
        return self.G.decode_pw(P, withprob=True)

    def decode_decoyvaults(self, Ps, grammarsize, pwsize, n_pw):
        # decode (grammarsize + n_pw * pwsize) random integers resulting in a decoy vault, n_pw is the number of passwords in the vault
        # Ps are bunch of random integers, the number of them should be the multiple of (grammarsize + n_pw * pwsize)
        decoyvault_size = grammarsize + n_pw * pwsize
        assert len(Ps) % (decoyvault_size) == 0, "Len(Ps) should be multiple of decoyvault_size"
        pcfg = TrainedGrammar()

        num_decoyvaults = len(Ps) // decoyvault_size
        decoy_vaults, probs_spm_mspm = [], []
        for ith_decoyv in tqdm.tqdm(range(num_decoyvaults)):
            try:
                ts = time.time()
                ith_devoyv_randints = Ps[ith_decoyv * decoyvault_size: (ith_decoyv + 1) * decoyvault_size]
                subgrammar_dte = DTE(pcfg.decode_grammar(ith_devoyv_randints[:grammarsize])) # HONEY_VAULT_GRAMMAR_SIZE = 500
                print(f"\ngrammar_decode used time:{time.time()-ts:.4f}s")
                
                pd_ts = time.time()
                dv = []
                for ith_pw_id in range(n_pw):
                    ith_pw_randints = ith_devoyv_randints[grammarsize + ith_pw_id * pwsize: grammarsize + (ith_pw_id + 1) * pwsize] # hny_config.PASSWORD_LENGTH = 100
                    ith_pw, ith_prob = subgrammar_dte.decode_pw(ith_pw_randints)
                    dv.append(ith_pw)
                    probs_spm_mspm.append([ith_prob,ith_prob])
                decoy_vaults.append(dv)
                print(f"password decode used time:{time.time()-pd_ts:.4f}s")
                print(f"one decoy vault decode used time:{time.time()-ts:.4f}s")
            except: # AssertionError
                continue
        return decoy_vaults, probs_spm_mspm

    def acquire_realvault_seed(self, pwlist):
        pcfg = TrainedGrammar()
        pcfgdte = DTE(pcfg.decode_grammar(random.randints(0, MAX_INT, 500)))
        nG = pcfgdte.G
        nG.update_grammar(*(pwlist))
        pcfgdte = DTE(nG)

        vaultseed = pcfg.encode_grammar(nG)
        for pw in pwlist:
            vaultseed.extend(pcfgdte.encode_pw(pw))
...
def main():
    # test decode_decoyvaults(self, Ps, grammarsize, pwsize, n_pw)
    pcfg = TrainedGrammar()
    dte = DTE(pcfg.decode_grammar(random.randints(0, MAX_INT, hny_config.HONEY_VAULT_GRAMMAR_SIZE)))
    '''grammarsize, pwsize = 500, 100
    n_pw = 100
    Ps = random.randints(0, MAX_INT, (grammarsize + pwsize*n_pw)*1000)
    decoy_vaults, probs_spm_mspm = dte.decode_decoyvaults(Ps, grammarsize, pwsize, n_pw)
    # iterate print
    for dv in decoy_vaults:
        print(dv)
    print("# of decoy vaults:", len(decoy_vaults))'''

    '''import json
    with open('/home/beeno/Dropbox/research_project/pycharm/incremental_vault_gpu/data/breachcompilation/fold2_bc100/fold_0_1.json', 'r') as f:
        bc = json.load(f)
    encode_error = 0
    for dv in list(bc.values()):
        vaultseed = dte.acquire_realvault_seed(dv)
        decoy_vault, probs_spm_mspm = dte.decode_decoyvaults(vaultseed, grammarsize, pwsize, len(dv))
    ...
    import numpy as np
    import json
    with open(SOURCE_PATH + '/data/breachcompilation/fold2_bc50/fold_0_1.json', 'r') as f:
        bc = json.load(f)
    for tset in tqdm.tqdm(list(bc.values())):
        if len(tset) > 5:
            continue
        pcfg = TrainedGrammar()
        while True:
        ...
        '''randomseeds = np.random.randint(low=0, high=MAX_INT, size=(500 + 100 * len(tset)) * 1000)  # *1.3 because some error for generating some decoy vaults
