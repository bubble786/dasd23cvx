...
from honeyvault_config import (HONEY_SERVER_URL, VAULT_FILE, STATIC_DOMAIN_HASH_LIST)
...
"""
    if len(args) < 3:
        return h_string
    mp = args[0]
    domain_pw_map = dict((get_exact_domain(k), v) for k, v in zip(args[1::2], args[2::2]))
...
"""
    ...
    vault_fl = args[1]
    g = lambda l: (l[0], l[-1])
    domain_pw_map = dict([g(a.strip().split(',')) for a in open(vault_fl) if a[0] != '#'])
    hv = HoneyVault(VAULT_FILE, mp, domain_pw_map)
    #hv.add_password(domain_pw_map)
    #if y.lower() == 'y':
    #hv.save()
    print("""
...
if __name__ == "__main__":
    import_vault(['AwesomeS@la', 'vault_file.csv'])